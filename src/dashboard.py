...
_AXIS_STYLE = dict(gridcolor="rgba(255,255,255,0.07)", zerolinecolor="rgba(255,255,255,0.1)")
_MARGIN     = dict(l=16, r=16, t=36, b=16)

def _apply_theme(fig, title: str = "", height: int = 0, margin: dict | None = None,
                 xrange=None, yrange=None):
    """Apply PLOTLY_THEME + axis styles in one call — avoids duplicate-kwarg errors."""
    kw = dict(**PLOTLY_THEME)
    if title:  kw["title"] = title
    if height: kw["height"] = height
    kw["margin"] = margin if margin else _MARGIN
    fig.update_layout(**kw)
    fig.update_xaxes(**_AXIS_STYLE, **({"range": xrange} if xrange else {}))
    fig.update_yaxes(**_AXIS_STYLE, **({"range": yrange} if yrange else {}))
    return fig
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        ["Overview", "EDA", "Model Comparison", "Feature Analysis", "Business Insights", "Test Coverage"],
...
if page == "Overview":
    ...
    with col_a:
        ...
        if train is not None and "FTR" in train.columns:
            ...
            _apply_theme(fig, title="Training set class distribution")
    ...
    with eda_tab4:
        if raw is None:
            ...
            if "H_ELO" in raw.columns and "A_ELO" in raw.columns:
                ...
                _apply_theme(fig_elo, title="ELO differential (Home − Away) distribution by outcome")
                fig_elo.update_layout(legend=dict(bgcolor="rgba(0,0,0,0)"))
            ...
            if "HxG" in raw.columns:
                ...
                _apply_theme(fig_xg, title="Home xG distribution by match outcome")
                fig_xg.update_layout(legend=dict(bgcolor="rgba(0,0,0,0)"))
            ...
            if "HST" in raw.columns:
                ...
                _apply_theme(fig_sot, title="Average shots-on-target differential (Home − Away) by outcome")
...
# PAGE: TEST COVERAGE
# ═══════════════════════════════════════════════════════════════════════════════
elif page == "Test Coverage":
    import xml.etree.ElementTree as ET

    st.title("Test Coverage Report")
    st.markdown(
        "<div style='color:#8a94b0;font-size:14px;margin-bottom:24px;'>"
        "Parsed live from <code>coverage.xml</code> — generated by "
        "<code>pytest --cov</code>.</div>",
        unsafe_allow_html=True,
    )
    ...
    cov_path = next((p for p in COVERAGE_PATHS if p.exists()), None)

    # Also accept an uploaded file
    uploaded_cov = st.file_uploader(
        "Or upload a coverage.xml file", type="xml", key="cov_upload"
    )

    @st.cache_data
    def parse_coverage(xml_bytes: bytes) -> dict:
        ...
        for pkg in root.iter("package"):
            ...
            for cls in pkg.iter("class"):
                ...
                total     = len(lines)
                missed    = [int(l.attrib["number"]) for l in lines
    ...
    xml_bytes = None
    if uploaded_cov is not None:
        xml_bytes = uploaded_cov.read()
    elif cov_path is not None:
        xml_bytes = cov_path.read_bytes()

    if xml_bytes is None:
        st.warning(
            "No `coverage.xml` found. Either place it in your project root and re-run, "
            "or upload it using the file picker above.",
            icon="⚠️",
        )
        st.code("pytest --cov=src --cov-report=xml tests/", language="bash")
        st.stop()

    cov = parse_coverage(xml_bytes)
    overall   = cov["overall"]
    packages  = cov["packages"]
    all_files = [cls for pkg in packages for cls in pkg["classes"]]

    pct = overall["line_rate"] * 100

    # ── Top metrics ────────────────────────────────────────────────────────────
    col_color = "#22c97e" if pct >= 80 else "#f5a623" if pct >= 60 else "#e85a5a"
    cols = st.columns(4)
    with cols[0]: metric_card("Overall Coverage",  f"{pct:.1f}%",
                               f"{overall['lines_covered']:,} / {overall['lines_valid']:,} lines", col_color)
    with cols[1]: metric_card("Lines Covered",  f"{overall['lines_covered']:,}", "executed lines", "#22c97e")
    with cols[2]: metric_card("Lines Missed",
                               f"{overall['lines_valid'] - overall['lines_covered']:,}",
                               "not executed", "#e85a5a")
    with cols[3]: metric_card("Modules",  str(len(all_files)), "files tracked", "#4d9fff")
    ...
    with col_g:
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number",
            value=pct,
            number={"suffix": "%", "font": {"size": 40, "color": col_color}},
            gauge={
                "axis": {"range": [0, 100], "tickcolor": "#5a6480",
                         "tickfont": {"color": "#5a6480"}},
                "bar":  {"color": col_color},
                "bgcolor": "#1a2030",
                "steps": [
                    {"range": [0,  60], "color": "rgba(232,90,90,0.15)"},
                    {"range": [60, 80], "color": "rgba(245,166,35,0.15)"},
                    {"range": [80,100], "color": "rgba(34,201,126,0.15)"},
                ],
                "threshold": {
            ...
            title={"text": "Overall line coverage", "font": {"color": "#8a94b0", "size": 14}},
        ))
    ...
    with col_p:
        st.subheader("Coverage by package")
        pkg_names = [p["name"] for p in packages]
        pkg_rates = [round(p["rate"] * 100, 1) for p in packages]
        pkg_colors = ["#22c97e" if r >= 80 else "#f5a623" if r >= 60 else "#e85a5a"
                      for r in pkg_rates]
        fig_pkg = go.Figure(go.Bar(
            x=pkg_rates,
            y=pkg_names,
            orientation="h",
            marker_color=pkg_colors,
            text=[f"{r}%" for r in pkg_rates],
            textposition="outside",
        ))
        _apply_theme(fig_pkg, title="Line coverage % per package", height=280)
        fig_pkg.update_xaxes(range=[0, 115], gridcolor="rgba(255,255,255,0.07)")
        st.plotly_chart(fig_pkg, use_container_width=True)

    st.divider()

    # ── Per-file bar chart ─────────────────────────────────────────────────────
    st.subheader("Coverage per file")

    sort_by = st.radio("Sort by", ["Coverage % (asc)", "Coverage % (desc)", "File name"],
                       horizontal=True, key="cov_sort")

    df_files = pd.DataFrame([{
        "File":     cls["name"],
        "Package":  next(p["name"] for p in packages if cls in p["classes"]),
        "Coverage": round(cls["rate"] * 100, 1),
        "Covered":  cls["covered"],
        "Total":    cls["total"],
        "Missed":   len(cls["missed"]),
    } for cls in all_files])

    if sort_by == "Coverage % (desc)":
        df_files = df_files.sort_values("Coverage", ascending=True)
    elif sort_by == "Coverage % (asc)":
        df_files = df_files.sort_values("Coverage", ascending=False)
    else:
        df_files = df_files.sort_values("File")

    bar_colors = ["#22c97e" if r >= 80 else "#f5a623" if r >= 60 else "#e85a5a"
                  for r in df_files["Coverage"]]
    ...
    fig_files = go.Figure(go.Bar(
        ...
        text=[f"{r}%  ({c}/{t})" for r, c, t in
              zip(df_files["Coverage"], df_files["Covered"], df_files["Total"])],
        textposition="outside",
        customdata=df_files[["Package", "Missed"]].values,
        hovertemplate="<b>%{y}</b><br>Package: %{customdata[0]}<br>"
                      "Coverage: %{x}%<br>Missed lines: %{customdata[1]}<extra></extra>",
    ))
    _apply_theme(fig_files, title="Line coverage % per module",
                 height=max(400, len(df_files) * 28),
                 margin=dict(l=16, r=80, t=36, b=16))
    fig_files.update_xaxes(range=[0, 125])
    st.plotly_chart(fig_files, use_container_width=True)

    st.divider()

    # ── Detailed file inspector ────────────────────────────────────────────────
    st.subheader("File inspector — missed lines")

    low_cov = [f for f in all_files if f["rate"] < 1.0 and f["missed"]]
    file_options = [f["name"] for f in sorted(low_cov, key=lambda x: x["rate"])]

    if not file_options:
        st.success("100% coverage across all files!")
    else:
        sel_file = st.selectbox(
            "Select file to inspect missed lines",
            file_options,
            format_func=lambda x: f"{x}  ({next(f['rate']*100 for f in all_files if f['name']==x):.1f}%)",
            key="cov_file_sel",
        )
        sel_data = next(f for f in all_files if f["name"] == sel_file)

        mc1, mc2, mc3 = st.columns(3)
        with mc1: metric_card("File Coverage", f"{sel_data['rate']*100:.1f}%", sel_file, col_color)
        with mc2: metric_card("Lines Covered", str(sel_data["covered"]), f"of {sel_data['total']} total", "#22c97e")
        with mc3: metric_card("Lines Missed",  str(len(sel_data["missed"])), "not executed", "#e85a5a")

        if sel_data["missed"]:
            missed_str = ", ".join(str(n) for n in sorted(sel_data["missed"]))
            st.markdown(
                f"<div style='background:#1a2030;border:1px solid rgba(232,90,90,0.3);"
                f"border-left:3px solid #e85a5a;border-radius:10px;padding:14px 18px;"
                f"font-family:IBM Plex Mono,monospace;font-size:12px;color:#e85a5a;"
                f"margin-top:8px;'>"
                f"<div style='color:#5a6480;font-size:10px;margin-bottom:6px;"
                f"text-transform:uppercase;letter-spacing:1px;'>Missed lines</div>"
                f"{missed_str}</div>",
                unsafe_allow_html=True,
            )
    ...
    def _cov_badge(val: float) -> str:
        pct_v = val * 100
        color = "#22c97e" if pct_v >= 80 else "#f5a623" if pct_v >= 60 else "#e85a5a"
        return f"{pct_v:.1f}%"

    display_df = df_files[["File", "Package", "Coverage", "Covered", "Total", "Missed"]].copy()

    def highlight_cov(val):
        if isinstance(val, float):
            color = "#22c97e" if val >= 80 else "#f5a623" if val >= 60 else "#e85a5a"
            return f"color: {color}"
        return ""

    st.dataframe(
        display_df.style.applymap(highlight_cov, subset=["Coverage"])
                        .format({"Coverage": "{:.1f}%"}),
        use_container_width=True,
        hide_index=True,
        height=400,
    ...
    st.caption(
        f"Generated from coverage.xml · "
        f"{overall['lines_covered']:,} / {overall['lines_valid']:,} lines · "
        f"Overall: {pct:.2f}%"
    )